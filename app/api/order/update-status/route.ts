...
export async function POST(request: NextRequest) {
  try {
    const { 
      ...
      description,
      userUid,
      expectedDeliveryDate, // New parameter for estimated delivery date
    } = await request.json();
    ...
    // Add expectedDeliveryDate if provided
    if (expectedDeliveryDate) {
      updateData.expectedDeliveryDate = expectedDeliveryDate instanceof Timestamp 
        ? expectedDeliveryDate 
        : Timestamp.fromDate(new Date(expectedDeliveryDate));
    }
