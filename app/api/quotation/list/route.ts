...
export async function GET(request: NextRequest) {
  try {
    ...
    // Note: When using multiple where clauses with orderBy and limit, Firestore requires composite indexes
    // To avoid index issues, we fetch all matching documents and let the client handle sorting/pagination
    let q = constraints.length > 0 
      ? query(collection(db, 'quotations'), ...constraints)
    ...
    for (const quotationDoc of querySnapshot.docs) {
      ...
      if (data.rfqId) {
        try {
          const rfqDoc = await getDoc(doc(db, 'rfqs', data.rfqId));
          if (rfqDoc.exists()) {
            const rfqData = rfqDoc.data();
            rfqTitle = rfqData.title || '';
            shipownerCompany = rfqData.shipownerCompany || '';
            vesselName = rfqData.vessel?.name || ''; // Get vessel name from RFQ
            rfqCategory = rfqData.mainCategory || rfqData.category || ''; // Get category from RFQ
          }
        } catch (error) {
          console.error(`Error fetching RFQ data for ${data.rfqId}:`, error);
        }
      }
      ...
      quotations.push({
        ...
        supplierRating,
        supplierReviewCount,
        estimatedReadyDate: data.estimatedReadyDate?.toDate?.()?.toISOString(), // Add estimated ready date
        expectedDeliveryDate: data.expectedDeliveryDate?.toDate?.()?.toISOString(), // Add expected delivery date
        createdAt: data.createdAt?.toDate?.()?.toISOString(),
