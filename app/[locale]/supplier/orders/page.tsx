...
export default function SupplierOrdersPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`/api/order/list?supplierUid=${user?.uid}${statusParam}`);
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-2">
              {locale === 'tr' 
                ? 'Aldığınız siparişleri yönetin ve takip edin'
                : 'Manage and track your received orders'}
            </p>
          </div>
                  ...
                  {filteredOrders.map((order) => (
                    <div key={order.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/${locale}/supplier/orders/${order.id}`} className="flex-1">
                        ...
                        {order.status === 'pending' && (
                          <Button className="flex-1">
                            {locale === 'tr' ? 'Onayla' : 'Confirm Order'}
                          </Button>
                        )}
                      </div>
                    </div>
